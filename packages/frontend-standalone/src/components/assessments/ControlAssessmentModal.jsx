...
import { assessControl } from '../../api/assessments';
...
const ControlAssessmentModal = ({
  ...
  useEffect(() => {
    console.log('🔍 Modal useEffect - existingAssessment:', existingAssessment);
    if (open && existingAssessment) {
      ...
      console.log('🔍 Modal resetting form with:', formData);
      reset(formData);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (!control?.subcategory_id) {
        const errorMsg = 'Control information is missing. Please close and reopen the modal.';
      ...
      await assessControl(assessmentId, payload);
      ...
      const errorMessage = err.response?.data?.message || 'Failed to save control assessment';
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      setEvidence([...evidence, response.data?.data]);
    } catch (err) {
