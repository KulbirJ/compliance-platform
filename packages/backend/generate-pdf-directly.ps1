Write-Host "`n====== Manual Report Generation Test ======`n" -ForegroundColor Cyan

Write-Host "Creating a test PDF report using Node.js directly...`n" -ForegroundColor Yellow

# Create a test Node.js script to generate the PDF
$testScript = @'
...
$testScript | Out-File -FilePath "generate-pdf-test.js" -Encoding UTF8

# Run the script
Write-Host "Generating PDF..." -ForegroundColor Yellow
node generate-pdf-test.js

# Check if file was created
if (Test-Path "test-compliance-report.pdf") {
    $fileInfo = Get-Item "test-compliance-report.pdf"
    Write-Host "`n✓ PDF generated successfully!" -ForegroundColor Green
    Write-Host "  File: test-compliance-report.pdf" -ForegroundColor Cyan
    Write-Host "  Size: $([math]::Round($fileInfo.Length / 1KB, 2)) KB" -ForegroundColor Cyan
    
    # Verify PDF
    $bytes = [System.IO.File]::ReadAllBytes("test-compliance-report.pdf")
    $header = [System.Text.Encoding]::ASCII.GetString($bytes[0..4])
    
    if ($header -eq "%PDF-") {
        Write-Host "  Format: Valid PDF" -ForegroundColor Green
    }
    
    Write-Host "`n====== Success! ======" -ForegroundColor Cyan
    Write-Host "`nOpen 'test-compliance-report.pdf' to view the report`n" -ForegroundColor White
} else {
    Write-Host "`n✗ PDF generation failed" -ForegroundColor Red
}

# Cleanup
Remove-Item "generate-pdf-test.js" -ErrorAction SilentlyContinue
