...
const assessmentModel = {
    ...
    try {
      const sql = `
        INSERT INTO compliance_assessments 
          (organization_id, created_by, assessment_name, description, scope, assessment_date, status)
        VALUES ($1, $2, $3, $4, $5, $6, $7)
        RETURNING id, organization_id, created_by, assessment_name, assessment_version, 
                  status, description, scope, assessment_date, due_date, completion_percentage, 
  ...
  async getAssessmentById(id) {
    try {
      console.log('DEBUG getAssessmentById: Received ID:', id, 'Type:', typeof id);
      const sql = `
        SELECT 
          ca.*,
      ...
      console.log('DEBUG getAssessmentById: Query returned', result.rows.length, 'rows');
      if (result.rows.length > 0) {
        console.log('DEBUG getAssessmentById: First row ID:', result.rows[0].id);
      }
      
      return result.rows.length > 0 ? result.rows[0] : null;
    } catch (error) {
      console.error('DEBUG getAssessmentById: Error:', error.message);
  ...
  async updateAssessment(id, updates) {
    try {
      const allowedFields = [
        'assessment_name', 'assessment_version', 'status', 'description', 'scope', 
    ...
    try {
      const sql = `
        SELECT 
          ...
          -- Completion percentage (assessed controls / total controls in assessment * 100)
          CASE 
            WHEN COUNT(DISTINCT cca.control_id) > 0 THEN
              ROUND((COUNT(DISTINCT CASE WHEN cca.implementation_status != 'not_implemented' 
                          THEN cca.control_id END)::DECIMAL / 
                    COUNT(DISTINCT cca.control_id)::DECIMAL * 100), 2)
            ELSE 0
  ...
  async updateCompletionPercentage(id) {
    try {
      // Get total controls and assessed controls for this assessment
      const statsQuery = await query(`
        SELECT 
          COUNT(*) as total_controls,
          COUNT(CASE WHEN implementation_status != 'not_implemented' THEN 1 END) as assessed_controls
        FROM compliance_control_assessments
        WHERE assessment_id = $1
      `, [id]);
      
      const stats = statsQuery.rows[0];
      const totalControls = parseInt(stats.total_controls) || 0;
      const assessedControls = parseInt(stats.assessed_controls) || 0;
      
      let completionPercentage = 0;
      if (totalControls > 0) {
        completionPercentage = Math.round((assessedControls / totalControls) * 100 * 100) / 100; // Round to 2 decimals
      }
      
      // Update the assessment
      const updateResult = await query(`
        UPDATE compliance_assessments 
        SET completion_percentage = $2, updated_at = CURRENT_TIMESTAMP
        WHERE id = $1
