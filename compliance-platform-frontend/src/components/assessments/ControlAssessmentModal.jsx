...
const STATUS_OPTIONS = [
  { value: 'compliant', label: 'Compliant / Implemented' },
  { value: 'partially_compliant', label: 'Partially Compliant' },
  { value: 'non_compliant', label: 'Non-Compliant / Not Implemented' },
...
const ControlAssessmentModal = ({
  ...
  useEffect(() => {
    if (open && existingAssessment) {
      reset({
        status: existingAssessment.status || 'not_assessed',
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to save control assessment');
    }
  };
