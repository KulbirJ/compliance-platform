...
const evidenceController = {
  ...
  async uploadEvidence(req, res, next) {
    try {
      ...
      // Get control assessment to verify it exists and get organization
  ...
  async getEvidenceList(req, res, next) {
    try {
      const controlAssessmentId = parseInt(req.query.controlAssessmentId);
      const userId = req.user.userId;

      if (!controlAssessmentId || isNaN(controlAssessmentId)) {
        return res.status(400).json({
          success: false,
          message: 'Control assessment ID is required'
        });
      }
      ...
      // Get evidence list (without file data)
      const evidenceList = await evidenceModel.getEvidenceByControlAssessment(
        controlAssessmentId
      );

      // Get evidence statistics
      const stats = await evidenceModel.getEvidenceStats(controlAssessmentId);

      res.json({
        success: true,
        data: {
          controlAssessmentId: controlAssessmentId,
          controlCode: controlAssessment.control_code,
          controlName: controlAssessment.control_name,
          totalEvidence: evidenceList.length,
          statistics: {
            totalCount: parseInt(stats.total_evidence) || 0,
            verifiedCount: parseInt(stats.verified_count) || 0,
            excellentQuality: parseInt(stats.excellent_quality) || 0,
            highQuality: parseInt(stats.high_quality) || 0,
            mediumQuality: parseInt(stats.medium_quality) || 0,
            lowQuality: parseInt(stats.low_quality) || 0,
            totalSizeBytes: parseInt(stats.total_size_bytes) || 0,
            totalSizeFormatted: formatFileSize(parseInt(stats.total_size_bytes) || 0)
          },
          evidence: evidenceList.map(e => ({
            id: e.id,
            evidenceName: e.evidence_name,
            fileType: e.file_type,
            fileSize: e.file_size,
            fileSizeFormatted: formatFileSize(e.file_size),
            evidenceType: e.evidence_type,
            qualityRating: e.quality_rating,
            description: e.description,
            isVerified: e.is_verified,
            verifiedBy: e.verified_by_name || e.verified_by_username,
            verifiedAt: e.verified_at,
            uploadedBy: e.uploaded_by_name || e.uploaded_by_username,
